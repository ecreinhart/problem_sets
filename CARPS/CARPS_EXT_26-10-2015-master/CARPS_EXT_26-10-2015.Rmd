...
```{r}
#sum of squares differences in R

options(contrasts=c("contr.sum","contr.poly"))


#regression model attempts 
summary(df_aov <- aov(recall_proportion ~ RISKCONDITION + recall_type + RISKCONDITION*recall_type + Error(`PARTICIPANT#ID`/RISKCONDITION), data = df))

summary(df_aov <- aov(recall_proportion ~ RISKCONDITION*recall_type, data = df))

df_aov <- aov(recall_proportion ~ RISKCONDITION*recall_type *Error(`PARTICIPANT#ID`/RISKCONDITION), data = df)

posthoc = TukeyHSD(df_aov, 'recall_type', conf.level=0.95)

lme_recall_prop = lme(recall_proportion ~ recall_type, data=scrd, random = ~1|Subject)
anova(lme_recall_prop)
...
#only select if recall_type = "RPplus" | "NRP"
df_subset_smaller = df_subset %>% filter(recall_type %in% c("NRP","RPplus"))

#regression attempts again 
summary(df_aov <- aov(recall_proportion ~ RISKCONDITION*recall_type, data = df_subset_smaller))

summary(df_aov <- aov(recall_proportion ~ RISKCONDITION + recall_type + RISKCONDITION*recall_type + Error(`PARTICIPANT#ID`/RISKCONDITION), data = df_subset_smaller))
...
#how to write csv
write.csv(df_subset, file = "data.csv")

#and add four figures, too 
